...
if mia_chiave is None:
    ...
    # Avviso sulle regole di ripetizione
    ...
    if game["modalita"] == "Colori":
        ...
        for i, val in enumerate(range(low, high + 1)):
            # Disabilita il tasto se la ripetizione non è ammessa ed è già stato premuto quel colore
            bottone_disabilitato = (not game["ripetizione_ammessa"] and str(val) in st.session_state.temp_key)
            if cols[i].button(COLOR_MAP[str(val)], key=f"key_set_{val}", disabled=bottone_disabilitato):
